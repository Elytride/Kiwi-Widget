...
// --- API Key ---
const GEMINI_API_KEY = "YOUR_GEMINI_API_KEY"; // please dont abuse my api key thanks.
...
async function fetchGeminiResponse(messageHistory) {
    if (!GEMINI_API_KEY || GEMINI_API_KEY === "YOUR_GEMINI_API_KEY") { return "AI response disabled: API Key not configured."; } const API_ENDPOINT = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`; const contents = messageHistory.map(msg => ({ role: msg.sender === 'user' ? 'user' : 'model', parts: [{ text: msg.text }] })); try { const response = await fetch(API_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: contents, "generationConfig": { "temperature": 0.7 } }) }); if (!response.ok) { const errorData = await response.json().catch(() => ({ message: response.statusText })); console.error("API Error Response:", errorData); throw new Error(`API Error ${response.status}: ${errorData?.error?.message || errorData.message || 'Unknown error'}`); } const data = await response.json(); const responseText = data?.candidates?.[0]?.content?.parts?.[0]?.text; if (!responseText) { console.warn("API response received, but no text found:", data); return "Received an empty or unexpected response from the AI."; } return responseText; } catch (error) { console.error("API Fetch Error:", error); return `Error communicating with AI: ${error.message}`; }
}
...
function KiwiWidget() {
    ...
    const handleDragStop = useCallback((e, d) => { window.getSelection()?.removeAllRanges(); setWidgetPosition({ x: d.x, y: d.y }); }, []);
    const handleResizeStop = useCallback((e, direction, ref, delta, position) => { setWidgetSize({ width: parseInt(ref.style.width, 10), height: parseInt(ref.style.height, 10), }); setWidgetPosition(position); }, []);
    ...
    return (
        <Rnd
            ref={rndRef}
            size={isExpanded ? widgetSize : MINIMIZED_SIZE}
            ...
            style={{ zIndex: 1000, background: 'transparent', border: 'none', overflow: 'visible', cursor: 'pointer' }}
            dragHandleClassName={isExpanded ? "widget-drag-handle" : "widget-drag-handle-minimized"}
        ...
        >
            ...
            {!isExpanded && (
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }} style={{ width: '100%', height: '100%' }}>
                    <Tooltip label={`KIWI (${themes[currentTheme].name}) - Right-click to expand`} placement="top">
                        <motion.div className="widget-drag-handle-minimized" style={{ width: '100%', height: '100%', cursor: 'grab', display: 'flex', alignItems: 'center', justifyContent: 'center' }} whileHover={{ scale: 1.05 }} animate={{ y: ["0px", "-4px", "0px"] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }} sx={{ '&:active': { cursor: 'grabbing' } }}>
            ...
            <AnimatePresence>
                {isExpanded && (
                    <motion.div key="expanded-widget" initial={{ opacity: 0, y: 30, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 30, scale: 0.9 }} transition={{ duration: 0.25, ease: "easeInOut" }} style={{ width: '100%', height: '100%' }}>
                        <Tabs index={activeTabIndex} onChange={handleTabChange} variant="unstyled" display="flex" flexDirection="column" height="100%" position="relative">
                            ...
                            <Box width="100%" height="100%" bg={colors.bg} borderWidth={colors.widgetBorderWidth || '2px'} borderStyle="solid" borderColor={colors.borderColor} borderRadius="xl" boxShadow="0 10px 25px rgba(0,0,0,0.1), 0 5px 10px rgba(0,0,0,0.08)" overflow="hidden" display="flex" flexDirection="column" pt={WIDGET_BODY_PADDING_TOP} position="relative">
                                {/* Top Bar - Draggable */}
                                <Flex className="widget-drag-handle" cursor="move" px={3} py={1} borderBottom={`1px solid ${colors.borderColor}`} alignItems="center" flexShrink={0} bg={colors.headerBg} position="relative" zIndex={5}>
                                    <Spacer />
                                    {/* Settings Menu */}
                                    <Menu>
                                        ...
                                        <MenuList
                                            ...
                                            borderColor="gray.200" // Use a light border
                                            zIndex={1500}
                                            color="black" // Force black text
                                            boxShadow="md" // Add a standard shadow
                                        >
                                            <MenuItem
                                                onClick={() => setCurrentTheme('light')}
                                                _hover={{ bg: "gray.100" }} // Force light gray hover
                                            >
                                                {themes.light.name} Theme
                                            </MenuItem>
                                            <MenuItem
                                                onClick={() => setCurrentTheme('dark')}
                                                _hover={{ bg: "gray.100" }} // Force light gray hover
                                            >
                                                {themes.dark.name} Theme
                                            </MenuItem>
                                            <MenuItem
                                                onClick={() => setCurrentTheme('midnight')}
                                                _hover={{ bg: "gray.100" }} // Force light gray hover
                                            >
                                                {themes.midnight.name} Theme
                                            </MenuItem>
                                        </MenuList>
                                    </Menu>
                                </Flex>
                                        ...
                                        <TabPanel key={session.id} p={0} display="flex" flexDirection="column" flexGrow={1} height="100%">
                                            ...
                                            <Box flex="1" overflowY="auto" p={4} ref={(el) => chatContainerRefs.current[session.id] = el} css={{ '&::-webkit-scrollbar': { width: '8px' }, '&::-webkit-scrollbar-track': { background: colors.scrollTrack, borderRadius: '4px' }, '&::-webkit-scrollbar-thumb': { background: colors.scrollThumb, borderRadius: '4px' }, '&::-webkit-scrollbar-thumb:hover': { background: colors.iconColor }, scrollbarWidth: 'thin', scrollbarColor: `${colors.scrollThumb} ${colors.scrollTrack}`, }}>
                                                <VStack spacing={3} align="stretch">
                                                    ...
                                                    {loadingStates[session.id] && (<Flex justify="flex-start" mt={2} pl={1}> <Spinner size="sm" color={colors.iconColor} mr={2} /> <Text fontSize="xs" color="gray.500">Kiwi is thinking...</Text> </Flex>)}
                                                </VStack>
                                            </Box>
